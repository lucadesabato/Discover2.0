...
def getStxSubType():
    ...
    if len(shigatoxin_typing) == 0:
        ...
        for subtype in shigatoxin_typing:
            blast_pident_100 = float(subtype[2]) == 100
            if (blast_pident_100):
...
def getSeroGroup(sero_file, antigen):
    ...
    for c in read:
        ...
        results=[]
        coverage=(int(line[2])/int(line[-1])*100)
        if coverage >= float(serotyper_v[2]) and float(line[8]) >= float(serotyper_v[3]) and coverage<=100.0:
            results.append(line[1])
            results.append(coverage)
            results.append(float(line[8]))
        if results:
    ...
    if list_antigen_sorted:
        serogroup = list_antigen_sorted[0][0][list_antigen_sorted[0][0].rfind(antigen):]
